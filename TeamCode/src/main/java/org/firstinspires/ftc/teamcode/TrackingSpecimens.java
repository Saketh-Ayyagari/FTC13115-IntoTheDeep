...
@TeleOp(name="TrackingSpecimens", group="Iterative OpMode")
//@Disabled
public class TrackingSpecimens extends OpMode {
...
//        drivetrain.powerChassisMotors(x_rotated, turn, y_rotated); // sends individual powers to the motors
        ...
        if(gamepad1.dpad_down){
            Point center = pipeline.get_contour_center();
            int actual = (int)center.y;
            final double SETPOINT = (CAMERA_HEIGHT/10);
            double align = PIDControl(SETPOINT, actual);

            enma.powerChassisMotors(align, 0, 0);
        }
